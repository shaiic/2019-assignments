...
class SimpleDataReader(object):
    ...
    def NormalizeX(self):
        ...
        feature_num = self.XRaw.shape[1]
        self.X_norm = np.zeros((feature_num,2))
        # 按列归一化,即所有样本的同一特征值分别做归一化
        for i in range(feature_num):
            # get one feature from all examples
            col_i = self.XRaw[:,i]
            max_val = np.max(col_i)
            min_val = np.min(col_i)
            # min value
            self.X_norm[i,0] = min_val 
            # range value
            self.X_norm[i,1] = max_val - min_val
            new_col = (col_i - self.X_norm[i,0])/(self.X_norm[i,1])
            X_new[:,i] = new_col
            #end for
        self.XTrain = X_new

    # normalize data by self range and min_val
    ...
    def NormalizeY(self):
        ...
        max_val = np.max(self.YRaw)
        min_val = np.min(self.YRaw)
        # min value
        self.Y_norm[0, 0] = min_val 
        # range value
        self.Y_norm[0, 1] = max_val - min_val 
        y_new = (self.YRaw - min_val) / self.Y_norm[0, 1]
        self.YTrain = y_new
...
if __name__ == '__main__':
    ...
    reader.ReadData()
    # 对输入数据进行归一化，采用min-max方法
    reader.NormalizeX()
    # 创建神经网络所需的基本配置参数，包括步长因子，最大epoch，批处理大小，允许误差等
    params = HyperParameters(eta=0.01, max_epoch=100, batch_size=10, eps = 1e-5)
    # 初始化神经网络模型，其中输入参数为两种类型，输出参数为1中类型
