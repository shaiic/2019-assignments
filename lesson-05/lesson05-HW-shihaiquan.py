...
file_name = "C:/Users/shq/Desktop/ML/ch05.npz"
...
class SimpleDataReader(object):
    def __init__(self):
        ...
        self.XTrain = None
        self.YTrain = None
        self.XRaw = None #训练数据X
        self.YRaw = None #训练数据Y
    ...
    def ReadData(self):
        ...
        if train_file.exists():
            ...
            self.num_train = self.XRaw.shape[0]   #训练数据X列数:x0 x1 x2 x3
    ...
    #归一化训练数据的X
    def NormalizeX(self):
        X_new = np.zeros(self.XRaw.shape)  #X归一化后的结果
        num_feature = self.XRaw.shape[1]   #X列数
        self.X_norm = np.zeros((num_feature,2))  #
        # 按列归一化,即所有样本的同一特征值分别做归一化
        for i in range(num_feature):
            # 取一列
            col_i = self.XRaw[:,i]
            max_value = np.max(col_i)  #最大值
            min_value = np.min(col_i)  #最小值
            # min value
            self.X_norm[i,0] = min_value
            # range value
            self.X_norm[i,1] = max_value - min_value
            new_col = (col_i - self.X_norm[i,0])/(self.X_norm[i,1])  #归一化
            X_new[:,i] = new_col  #存放某列的结果
        #end for
        self.XTrain = X_new
...
if __name__ == '__main__':
    ...
    x_new = reader.NormalizePredicateData(x) #归一化预测的X
    print(x_new)
