...
cd $HOME_DIR
# echo clone t2t v1.2.9 to localpath...
...
# echo clone finished.
cp -r /root/tensor2tensor/ $HOME_DIR/tensor2tensor-1.2.9
...
python3 -m pip install -r pip_requirements_gpu.txt

# generate data
echo start generate data...
python3 ${binFile}/t2t-datagen \
  --t2t_usr_dir=${USR_DIR} \
  --data_dir=${DATA_DIR} \
  --problem=${PROBLEM}
