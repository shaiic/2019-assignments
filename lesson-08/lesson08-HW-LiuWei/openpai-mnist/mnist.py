...
WORK_DIRECTORY = None
IMAGE_SIZE = 28#mnist图片大小28*28
NUM_CHANNELS = 1#黑白图片只有一个通道 
PIXEL_DEPTH = 255#灰度取值范围0-255
NUM_LABELS = 10#标签值个数10个（0--9共10个数字）
VALIDATION_SIZE = 5000  # Size of the validation set.取5000个验证值
SEED = 66478  # Set to None for random seed.
BATCH_SIZE = 64#一次取多少个图片
NUM_EPOCHS = 10#数据几遍
EVAL_BATCH_SIZE = 64
...
def extract_data(filename, num_images):
  ...
  with gzip.open(filename) as bytestream:
    bytestream.read(16)#16个字节是文件头
    buf = bytestream.read(IMAGE_SIZE * IMAGE_SIZE * num_images * NUM_CHANNELS)#取的数据个数28*28*60000*1
    data = numpy.frombuffer(buf, dtype=numpy.uint8).astype(numpy.float32)#转成浮点
    data = (data - (PIXEL_DEPTH / 2.0)) / PIXEL_DEPTH#归一化数据-0.5到0.5之间
    data = data.reshape(num_images, IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNELS)#把一维数据数组调整成60000个图片
    return data
...
def main(_):
  ...
  conv1_weights = tf.Variable(
      tf.truncated_normal([5, 5, NUM_CHANNELS, 32],  # 5x5 filter, depth 32.
                          stddev=0.1,
                          seed=SEED, dtype=data_type()))#通过正态分布初始化第一层过滤器5*5*1*32，因为只有一个通道，可以理解为32个神经元
  conv1_biases = tf.Variable(tf.zeros([32], dtype=data_type()))
  conv2_weights = tf.Variable(tf.truncated_normal(
      [5, 5, 32, 64], stddev=0.1,
      seed=SEED, dtype=data_type()))#通过正态分布初始化第二层过滤器5*5*32*64，32代表第一层有32个神经元，64代表第二层有64个神经元
  ...
  def model(data, train=False):
    ...
    conv = tf.nn.conv2d(data,
                        conv1_weights,
                        strides=[1, 1, 1, 1],#index, y, x, depth
                        padding='SAME')#卷积的X*W+B
    # Bias and rectified linear non-linearity.
    relu = tf.nn.relu(tf.nn.bias_add(conv, conv1_biases))#激活函数reLU
